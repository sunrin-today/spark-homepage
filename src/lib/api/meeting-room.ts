import { ListResponse } from "@/types/common";
import api from "./api"
import { MeetingRoomRequest, MeetingRoomRequestPost } from "@/types/meeting-room";

const meetingRoomApi = {
    getMeetingRoomSchedule: async ({month, limit = 10, page = 1, column, orderDirection}: {month?: number, limit?: number, page?: number, column: string, orderDirection: string}) => {
        const response = await api.get<ListResponse<MeetingRoomRequest>>("/api/meeting-room/me", {
            params: {
                month,
                limit,
                page,
                column,
                orderDirection
            }
        });
        return response;
    },
    postMeetingRoomRequest: async (data : MeetingRoomRequestPost) => {
        const response = await api.post("/api/meeting-room", data);
        return response;
    },
}

export default meetingRoomApi